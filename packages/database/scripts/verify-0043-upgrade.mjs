...
if (!databaseUrlValue) {
  throw new Error('DATABASE_URL is required for Task 0043 upgrade verification');
}

const databaseUrl = new URL(databaseUrlValue);
const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const sourceMigrations = join(packageRoot, 'prisma', 'migrations');
const upgradeMigration = '20260922170000_task_0043_pharmacy_pos_billing_gst_invoice';
...
function databaseName() {
  return `medsphere_0043_upgrade_${randomUUID().replaceAll('-', '').slice(0, 12)}`;
}
...
function createMigrationProject() {
  const projectRoot = mkdtempSync(join(tmpdir(), 'medsphere-0043-upgrade-'));
...
const tenantId = '10000000-0000-4000-8000-000000004301';
const userId = '20000000-0000-4000-8000-000000004301';
const membershipId = '30000000-0000-4000-8000-000000004301';
const providerId = '40000000-0000-4000-8000-000000004301';
const productId = '50000000-0000-4000-8000-000000004301';
const inventoryId = '60000000-0000-4000-8000-000000004301';
const batchId = '70000000-0000-4000-8000-000000004301';
const movementId = '80000000-0000-4000-8000-000000004301';
const accessId = '90000000-0000-4000-8000-000000004301';
const identifierId = 'a0000000-0000-4000-8000-000000004301';
const importJobId = 'b0000000-0000-4000-8000-000000004301';
const importRowId = 'c0000000-0000-4000-8000-000000004301';
const importReceiptId = 'd0000000-0000-4000-8000-000000004301';
const reservationId = 'e0000000-0000-4000-8000-000000004301';
const reservationItemId = 'e0000000-0000-4000-8000-000000004302';
const reservationAllocationId = 'e0000000-0000-4000-8000-000000004303';
const posAuditId = 'f0000000-0000-4000-8000-000000004301';

const baselineSeedSql = `
INSERT INTO "Tenant"
  ("id","name","slug","organizationType","isActive","selfRegistrationEnabled","version","createdAt","updatedAt")
VALUES
  ('${tenantId}','Task 0043 Upgrade Pharmacy','task-0043-upgrade','PHARMACY',true,false,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);

INSERT INTO "User"
  ("id","email","passwordHash","firstName","lastName","preferredLanguage","status","version","createdAt","updatedAt")
VALUES
  ('${userId}','task0043-upgrade@medsphere.test','fixture-not-a-real-credential','Task','0043','en','ACTIVE',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
VALUES
  ('${providerId}','${tenantId}','PHARMACY','Task 0043 Upgrade Pharmacy','Fixture Owner','task0043-provider@medsphere.test','0000043001','Fixture address','Chennai','Tamil Nadu','India','600001',13.0827,80.2707,true,true,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
VALUES
  ('${productId}','Task 0043 Preserved Medicine','Preserved Generic','Preserved Brand','MEDICINE','Fixture Manufacturer','TABLET','500 mg','4006381333931',false,true,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
VALUES
  ('${inventoryId}','${tenantId}','${providerId}','${productId}','PRESERVED-0043',12.00,15.00,2.50,5.00,4,true,3,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
VALUES
  ('${batchId}','${tenantId}','${inventoryId}','${providerId}','${productId}','PRESERVED-BATCH-0043','2026-01-01','2030-01-01',20,17,2,10.00,12.00,'ACTIVE',5,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
VALUES
  ('${movementId}','${tenantId}','${inventoryId}','${batchId}','${providerId}','${productId}','STOCK_IN',20,0,20,'inventory.batch.receive','${batchId}','Task 0043 upgrade fixture','task-0043-preserved-movement',NULL,1,'TENANT_USER','${membershipId}',CURRENT_TIMESTAMP - INTERVAL '1 day',CURRENT_TIMESTAMP - INTERVAL '1 day');
...
VALUES
  ('${importJobId}','${tenantId}','${providerId}','${membershipId}','${membershipId}','CSV','preserved.csv',repeat('a',64),'{"Barcode":"identifier"}'::jsonb,'task-0043-preserved-stage',repeat('b',64),'task-0043-preserved-apply',repeat('c',64),'APPLIED',1,1,0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
BEGIN
  ...
  IF (SELECT count(*) FROM "InventoryImportJob" WHERE "id" = '${importJobId}' AND "status" = 'APPLIED') <> 1
    ...
    RAISE EXCEPTION 'Task 0043 upgrade mutated accepted Task 0042 import evidence';
  END IF;
  ...
  IF (SELECT count(*) FROM "MedicineReservation" WHERE "id" = '${reservationId}' AND "status" = 'READY' AND "version" = 2) <> 1
    ...
    RAISE EXCEPTION 'Task 0043 upgrade mutated existing reservation evidence';
  END IF;

  IF (SELECT count(*) FROM "PharmacySale") <> 0
     OR (SELECT count(*) FROM "PharmacyInvoice") <> 0
     OR (SELECT count(*) FROM "PharmacySalePayment") <> 0 THEN
    RAISE EXCEPTION 'Task 0043 upgrade fabricated POS transaction history';
  END IF;
  ...
  IF (SELECT count(*) FROM "Permission" WHERE "name" IN ('billing.pos.read','billing.pos.checkout','billing.pos.configure','billing.pos.void')) <> 4 THEN
    RAISE EXCEPTION 'Task 0043 POS permission catalogue is incomplete';
  END IF;
  ...
  IF migration_count <> 1 THEN
    RAISE EXCEPTION 'Task 0043 migration is not recorded exactly once: %', migration_count;
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM "_prisma_migrations"
    ...
    RAISE EXCEPTION 'Task 0043 repeated deploy changed migration history';
  END IF;
  IF (SELECT count(*) FROM "Inventory" WHERE "id" = '${inventoryId}') <> 1
    ...
    RAISE EXCEPTION 'Task 0043 repeated deploy changed accepted data';
  END IF;
END $$;
`,
    );

    process.stdout.write('Task 0043 populated upgrade verification passed.\n');
  } finally {
    try {
