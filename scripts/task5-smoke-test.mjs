...
async function main() {
  ...
  bootstrapUncreatableFoundationState({ tenantId, tenantSlug, providerAId, providerBId, roleId, productId });
  ...
  record('registration (accepted API)', adminRegister.status === 200 && staffRegister.status === 200 ? 'WORKING' : 'BROKEN', `admin=${adminRegister.status} staff=${staffRegister.status}`);

  bootstrapMembershipActivation({ tenantId, adminEmail, staffEmail, roleId });
  record('membership activation + first admin role grant (documented gap, test-only bootstrap)', 'PARTIAL', 'no accepted self-service verification path exists yet (Batch 2 Task 2 finding); this is bootstrap only, not a working product flow', { allowedPartial: true });
  ...
  if (adminLogin.res.status !== 200 || staffLogin.res.status !== 200 || !adminLogin.accessToken) {
    record('authentication + authenticated session', 'BROKEN', `admin=${adminLogin.res.status} staff=${staffLogin.res.status}`);
    return;
  }
  ...
  for (const providerId of [providerAId, providerBId]) {
    const grant = await fetch(`${BACKEND}/authorization/memberships/${adminMembershipId}/provider-access/${providerId}`, { method: 'PUT', headers: { authorization: `Bearer ${adminLogin.accessToken}` } });
    if (!grant.ok) providerAccessOk = false;
  }
  record('provider-access grant (accepted backend API)', providerAccessOk ? 'WORKING' : 'BROKEN', providerAccessOk ? '' : 'one or more provider-access grants failed');

  const adminCatalogue = await fetch(`${FRONTEND}/api/authorization/catalogue`, { headers: { cookie: adminLogin.cookie } });
  ...
  record('RBAC allow (admin reads authorization catalogue)', adminCatalogue.status === 200 && catalogueHasPermissions ? 'WORKING' : 'BROKEN', `status ${adminCatalogue.status}, permissions present: ${Boolean(catalogueHasPermissions)}`);

  const staffStockAttempt = await fetch(`${FRONTEND}/api/inventory/stock?providerId=${providerAId}&limit=1&offset=0`, { headers: { cookie: staffLogin.cookie } });
  record('RBAC deny (no-role staff account denied inventory read)', staffStockAttempt.status === 403 ? 'WORKING' : 'BROKEN', `expected 403, got ${staffStockAttempt.status}`);

  const dashboardPage = await fetch(`${FRONTEND}/dashboard`, { headers: { cookie: adminLogin.cookie }, redirect: 'manual' });
  const dashboardHtml = dashboardPage.status === 200 ? await dashboardPage.text() : '';
  const dashboardHasOperationalContent = dashboardHtml.includes('Stock records') || dashboardHtml.includes('Reservation records');
  record('dashboard page (authenticated, real operational content)', dashboardPage.status === 200 && dashboardHasOperationalContent ? 'WORKING' : 'BROKEN', `status ${dashboardPage.status}, operational markers present: ${dashboardHasOperationalContent}`);

  assertProcessesAlive('inventory listing + batch receipt');
  const configureInventory = await fetch(`${BACKEND}/inventory/providers/${providerAId}/products/${productId}`, {
    method: 'PUT', headers: { 'content-type': 'application/json', authorization: `Bearer ${adminLogin.accessToken}` },
    body: JSON.stringify({ sellingPrice: '20.00', mrp: '25.00', discountPercentage: '0.00', taxPercentage: '0.00', idempotencyKey: `task5-smoke-listing-${randomUUID()}` }),
  });
  record('inventory listing creation (accepted configureInventory API)', configureInventory.status === 200 ? 'WORKING' : 'BROKEN', `status ${configureInventory.status}`);
  ...
  async function receiveBatch(batchNumber, quantity) {
    return fetch(`${BACKEND}/inventory/providers/${providerAId}/products/${productId}/batches`, {
      method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${adminLogin.accessToken}` },
      body: JSON.stringify({ batchNumber, expiryDate, quantity, purchasePrice: '15.00', sellingPrice: '20.00', idempotencyKey: `task5-smoke-receive-${randomUUID()}` }),
    });
  }
  ...
  record('batch receipt (accepted receiveBatch API)', receiveBatch1.status === 200 && receiveBatch2.status === 200 && batchId && batch2Id ? 'WORKING' : 'BROKEN', `status ${receiveBatch1.status}/${receiveBatch2.status}`);
  if (!batchId || !batch2Id) return;

  const stockRead = await fetch(`${FRONTEND}/api/inventory/stock?providerId=${providerAId}&limit=10&offset=0`, { headers: { cookie: adminLogin.cookie } });
  const stockBody = await json(stockRead);
  const stockMatches = stockBody?.data?.some((item) => item.name === 'Task5 Smoke Paracetamol' && item.batches?.some((b) => b.id === batchId && b.onHandQuantity === 20));
  record('inventory read (real product/batch/quantities present)', stockRead.status === 200 && stockMatches ? 'WORKING' : 'BROKEN', `status ${stockRead.status}, matched seeded batch: ${Boolean(stockMatches)}`);

  const quarantineRequestId = `task5-smoke-quarantine-${randomUUID()}`;
  const quarantine = await fetch(`${FRONTEND}/api/inventory/providers/${providerAId}/batches/${batchId}/quarantine`, {
    method: 'POST', headers: { 'content-type': 'application/json', origin: FRONTEND, cookie: adminLogin.cookie, 'x-request-id': quarantineRequestId },
    body: JSON.stringify({ expectedVersion: 1, idempotencyKey: `task5-smoke-quarantine-key-${randomUUID()}`, reasonCode: 'QUALITY_SUSPECT' }),
  });
  const quarantineBody = await json(quarantine);
  const quarantineStateOk = quarantineBody?.status === 'QUARANTINED' && quarantineBody?.batchId === batchId;
  record('inventory mutation (quarantine actually mutated batch state)', quarantine.status === 200 && quarantineStateOk ? 'WORKING' : 'BROKEN', `status ${quarantine.status}, reported status: ${quarantineBody?.status}`);

  const auditRead = await fetch(`${FRONTEND}/api/audit/events?resourceId=${batchId}&limit=5`, { headers: { cookie: adminLogin.cookie } });
  const auditBody = await json(auditRead);
  const auditEvent = auditBody?.data?.find((e) => e.eventType === 'inventory.batch.quarantined');
  const auditMatches = auditEvent && auditEvent.outcome === 'SUCCEEDED' && auditEvent.resourceId === batchId && auditEvent.resourceType === 'Batch' && typeof auditEvent.actorMembershipId === 'string' && auditEvent.requestId === quarantineRequestId;
  record('audit evidence (tenant/actor/resource/action/correlation all match)', auditRead.status === 200 && auditMatches ? 'WORKING' : 'BROKEN', `status ${auditRead.status}, event found: ${Boolean(auditEvent)}, correlation matched: ${auditEvent?.requestId === quarantineRequestId}`);

  assertProcessesAlive('reservation + transfer + public search');
  const publicSearch = await fetch(`${FRONTEND}/api/public/providers/${providerAId}/medicine-search?q=Paracetamol&limit=10&offset=0`);
  const publicSearchBody = await json(publicSearch);
  const publicResult = publicSearchBody?.data?.find((r) => r.productId === productId);
  const publicSearchSemanticsOk = publicResult && publicResult.name === 'Task5 Smoke Paracetamol' && publicResult.providerId === providerAId && publicResult.availability === 'IN_STOCK' && !('inventoryId' in publicResult) && !('purchasePrice' in publicResult) && !('sellingPrice' in publicResult) && !('ownerName' in publicResult) && !('email' in publicResult);
  record('public medicine search (correct data, no forbidden/private fields)', publicSearch.status === 200 && publicSearchSemanticsOk ? 'WORKING' : 'BROKEN', `status ${publicSearch.status}, result found and field-minimized: ${Boolean(publicSearchSemanticsOk)}`);

  const staffUserId = sql(`SELECT id FROM "User" WHERE email = '${staffEmail}';`);
  const reservationCreate = await fetch(`${FRONTEND}/api/inventory/providers/${providerAId}/reservations`, {
    method: 'POST', headers: { 'content-type': 'application/json', origin: FRONTEND, cookie: adminLogin.cookie },
    body: JSON.stringify({ subjectUserId: staffUserId, expiresAt: new Date(Date.now() + 3_600_000).toISOString(), items: [{ productId, quantity: 2 }], idempotencyKey: `task5-smoke-reservation-${randomUUID()}` }),
  });
  const reservationBody = await json(reservationCreate);
  const reservationId = reservationBody?.reservationId;
  const reservationCreateOk = reservationBody?.status === 'PENDING' && reservationBody?.totalQuantity === 2 && reservationBody?.itemCount === 1;
  record('staff-assisted reservation creation (correct provider/product/quantity/status)', reservationCreate.status === 200 && reservationCreateOk ? 'WORKING' : 'BROKEN', `status ${reservationCreate.status}, semantics ok: ${Boolean(reservationCreateOk)}`);
  if (!reservationId) return;

  const confirm = await fetch(`${FRONTEND}/api/inventory/providers/${providerAId}/reservations/${reservationId}/transitions`, {
    method: 'POST', headers: { 'content-type': 'application/json', origin: FRONTEND, cookie: adminLogin.cookie },
    body: JSON.stringify({ transition: 'CONFIRM', expectedVersion: 1, idempotencyKey: `task5-smoke-confirm-${randomUUID()}` }),
  });
  const confirmBody = await json(confirm);
  const ready = await fetch(`${FRONTEND}/api/inventory/providers/${providerAId}/reservations/${reservationId}/transitions`, {
    method: 'POST', headers: { 'content-type': 'application/json', origin: FRONTEND, cookie: adminLogin.cookie },
    body: JSON.stringify({ transition: 'READY', expectedVersion: 2, idempotencyKey: `task5-smoke-ready-${randomUUID()}` }),
  });
  const readyBody = await json(ready);
  const lifecycleOk = confirm.status === 200 && confirmBody?.status === 'CONFIRMED' && ready.status === 200 && readyBody?.status === 'READY';
  record('reservation lifecycle (CONFIRM -> READY, correct resulting status each step)', lifecycleOk ? 'WORKING' : 'BROKEN', `confirm status ${confirm.status}/${confirmBody?.status}, ready status ${ready.status}/${readyBody?.status}`);

  const transfer = await fetch(`${FRONTEND}/api/inventory/providers/${providerAId}/transfers`, {
    method: 'POST', headers: { 'content-type': 'application/json', origin: FRONTEND, cookie: adminLogin.cookie },
    body: JSON.stringify({ destinationProviderId: providerBId, sourceBatchId: batch2Id, expectedSourceVersion: 1, quantity: 3, idempotencyKey: `task5-smoke-transfer-${randomUUID()}` }),
  });
  const transferBody = await json(transfer);
  const transferOk = transferBody?.sourceOnHandAfter === 7 && transferBody?.destinationOnHandAfter === 3 && transferBody?.sourceProviderId === providerAId && transferBody?.destinationProviderId === providerBId;
  record('stock transfer (correct source/target quantities)', transfer.status === 200 && transferOk ? 'WORKING' : 'BROKEN', `status ${transfer.status}, source after: ${transferBody?.sourceOnHandAfter}, destination after: ${transferBody?.destinationOnHandAfter}`);

  assertProcessesAlive('notification path verification');
  const providerConfigured = typeof process.env.NOTIFICATION_EMAIL_PROVIDER_ENABLED !== 'undefined' || typeof process.env.NOTIFICATION_EMAIL_PROVIDER_KEY !== 'undefined';
  if (providerConfigured) {
    record('notification path (disabled-safe configuration)', 'BROKEN', 'NOTIFICATION_EMAIL_PROVIDER_* is set in this run; this workflow must never configure a real provider');
  } else {
    const deliveryRows = sql(`SELECT status FROM "NotificationDelivery" WHERE "tenantId" = '${tenantId}';`);
    const rows = deliveryRows ? deliveryRows.split('\n').filter(Boolean) : [];
    const anySucceeded = rows.some((status) => status === 'SUCCEEDED' || status === 'DELIVERED');
    record('notification path (proven fail-closed, no provider configured, no external delivery)', anySucceeded ? 'BROKEN' : 'WORKING', anySucceeded ? 'a delivery row reports success with no provider configured -- impossible unless something bypassed the deny-by-default contract' : `${rows.length} delivery row(s) recorded for this tenant, none report a successful send; NOTIFICATION_EMAIL_PROVIDER_* was never set in this run`);
  }
}
