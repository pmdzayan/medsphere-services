...
describe('G3.11 batch-quarantine HTTP contract', () => {
  it('uses only the accepted protected private no-store route', () => {
    const handler = InventoryController.prototype.quarantineBatch;
    expect(Reflect.getMetadata(PATH_METADATA, handler)).toBe(
      'providers/:providerId/batches/:batchId/quarantine',
    );
