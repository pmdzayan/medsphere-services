...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid inventory import preview is required.' }, 400);
  }
  if (!isStageInventoryImportRequest(command)) {
    return privateNoStore({ message: 'A valid inventory import preview is required.' }, 400);
  }
  ...
  try {
    ...
    const upstream = await fetch(
      authApiUrl(`/inventory/providers/${encodeURIComponent(providerId)}/imports/stage`),
      { method: 'POST', headers, body: JSON.stringify(command), cache: 'no-store' },
    );
    if (!upstream.ok) {
      const message = await boundedUpstreamMessage(upstream, 'Unable to stage inventory import.');
      return privateNoStore({ message }, mutationStatus(upstream.status));
    }
    const payload: unknown = await upstream.json();
    return isInventoryImportPreview(payload) && payload.providerId === providerId
