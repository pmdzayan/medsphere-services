...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    const upstream = await fetch(
      authApiUrl(`/inventory/providers/${encodeURIComponent(providerId)}/availability-requests/${encodeURIComponent(requestId)}/responses`),
      { method: 'POST', headers, body: JSON.stringify(command), cache: 'no-store' },
    );
    if (!upstream.ok) {
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to respond to availability request.') },
        [401,403,404,409,412,428].includes(upstream.status) ? upstream.status : upstream.status >= 500 ? 502 : 400,
      );
    }
