...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid quarantine command is required.' }, 400);
  }
  if (!isBatchQuarantineRequest(command)) {
    return privateNoStore({ message: 'A valid quarantine command is required.' }, 400);
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      const message = await boundedUpstreamMessage(upstream, 'Unable to quarantine this batch.');
    ...
    return isBatchQuarantineResponse(receipt)
