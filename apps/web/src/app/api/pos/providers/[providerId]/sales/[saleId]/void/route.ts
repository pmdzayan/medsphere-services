...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid POS void command is required.' }, 400);
  }
  if (!isPosVoidRequest(command))
    return privateNoStore({ message: 'A valid POS void command is required.' }, 400);
  try {
    ...
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to void POS sale.') },
