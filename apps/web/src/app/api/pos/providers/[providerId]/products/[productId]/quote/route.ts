...
export async function GET(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  if (!isCanonicalUuid(providerId) || !isCanonicalUuid(productId))
    return privateNoStore(
      { message: 'A valid provider and product identifier are required.' },
      400,
    );
  ...
  try {
    ...
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to load POS quote.') },
        status(upstream.status),
      );
    const payload: unknown = await upstream.json();
    return isPosProductQuote(payload) &&
