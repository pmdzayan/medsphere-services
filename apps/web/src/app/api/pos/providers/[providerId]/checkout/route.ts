...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid POS checkout is required.' }, 400);
  }
  if (!isPosCheckoutRequest(command))
    return privateNoStore({ message: 'A valid POS checkout is required.' }, 400);
  try {
    const headers = upstreamHeaders(request, token);
    headers.set('content-type', 'application/json');
    const upstream = await fetch(
      authApiUrl('/pos/providers/' + encodeURIComponent(providerId) + '/checkout'),
      { method: 'POST', headers, body: JSON.stringify(command), cache: 'no-store' },
    );
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to complete POS checkout.') },
