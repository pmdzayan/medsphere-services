...
export async function GET(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    const upstream = await fetch(
      authApiUrl(
        '/pos/providers/' + encodeURIComponent(providerId) + '/sales/' + encodeURIComponent(saleId),
      ),
      { headers: upstreamHeaders(request, token), cache: 'no-store' },
    );
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to load POS sale.') },
        status(upstream.status),
      );
    const payload: unknown = await upstream.json();
    return isPosSaleReceipt(payload) &&
