...
export async function GET(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    const upstream = await fetch(
      authApiUrl('/pos/providers/' + encodeURIComponent(providerId) + '/fiscal-profile'),
      { headers: upstreamHeaders(request, token), cache: 'no-store' },
    );
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to load POS fiscal profile.') },
...
export async function PUT(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid POS fiscal profile is required.' }, 400);
  }
  if (!isPosFiscalProfileRequest(command))
    return privateNoStore({ message: 'A valid POS fiscal profile is required.' }, 400);
  try {
    ...
    const upstream = await fetch(
      authApiUrl('/pos/providers/' + encodeURIComponent(providerId) + '/fiscal-profile'),
      { method: 'PUT', headers, body: JSON.stringify(command), cache: 'no-store' },
    );
    if (!upstream.ok)
      return privateNoStore(
        { message: await boundedUpstreamMessage(upstream, 'Unable to update POS fiscal profile.') },
        status(upstream.status),
      );
    const payload: unknown = await upstream.json();
...
function status(value: number): number {
  return [401, 403, 404, 409].includes(value) ? value : value >= 500 ? 502 : 400;
}
function privateNoStore(body: unknown, code: number): NextResponse {
