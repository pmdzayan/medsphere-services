...
import {
  isPosInventoryFiscalConfigurationResult,
  isPosInventoryFiscalProfileRequest,
...
export async function PUT(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  if (!isCanonicalUuid(providerId) || !isCanonicalUuid(inventoryId))
    return privateNoStore(
      { message: 'A valid provider and inventory identifier are required.' },
      400,
    );
  ...
  try {
    ...
    return privateNoStore({ message: 'A valid inventory fiscal profile is required.' }, 400);
  }
  if (!isPosInventoryFiscalProfileRequest(command))
    return privateNoStore({ message: 'A valid inventory fiscal profile is required.' }, 400);
  try {
      ...
      return privateNoStore(
        {
          message: await boundedUpstreamMessage(
            upstream,
            'Unable to update inventory fiscal profile.',
          ),
        },
        status(upstream.status),
      );
    const payload: unknown = await upstream.json();
    return isPosInventoryFiscalConfigurationResult(payload) &&
