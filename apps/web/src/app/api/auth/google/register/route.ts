...
import {
  isGoogleRegisterRequest,
  isRegistrationResponse,
  normalizeGoogleRegisterRequest,
  validateGoogleRegisterRequest,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return noStore({ message: 'Invalid Google onboarding request.' }, 400);
  }

  if (!isGoogleRegisterRequest(payload)) {
    return noStore({ message: 'Invalid Google onboarding request.' }, 400);
  }

  const registration = normalizeGoogleRegisterRequest(payload);

  if (Object.keys(validateGoogleRegisterRequest(registration)).length > 0) {
    return noStore({ message: 'Invalid Google onboarding request.' }, 400);
  }
  ...
  try {
    ...
    upstream = await fetch(authApiUrl('/auth/google/register'), {
      method: 'POST',
      headers,
