...
describe('Google registration BFF boundary', () => {
  ...
  it('rejects invalid and over-broad Google onboarding requests', async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);

    const invalid = await POST(request({ ...validRequest, idToken: '' }));
    const overBroad = await POST(
      request({ ...validRequest, tenantId: 'client-controlled-tenant' }),
    );
  ...
  it('normalizes and forwards only the accepted Google registration contract', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
    ...
    expect(url).toContain('/auth/google/register');
...
function request(body: unknown, origin = 'http://localhost'): NextRequest {
  return new NextRequest('http://localhost/api/auth/google/register', {
    method: 'POST',
    headers: {
