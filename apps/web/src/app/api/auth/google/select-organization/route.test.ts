...
describe('Google organization selection session boundary', () => {
  ...
  it('rejects bare, malformed, empty-token, or over-broad membership selection', async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);

    const responses = await Promise.all([
      POST(createRequest({ membershipId: validRequest.membershipId })),
      POST(createRequest({ ...validRequest, tenantId: 'client-controlled-tenant' })),
      POST(createRequest({ ...validRequest, membershipId: 'garbage' })),
      POST(createRequest({ ...validRequest, idToken: '' })),
    ]);
