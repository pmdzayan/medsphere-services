...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!refreshToken) {
    const response = noStoreJson({ message: 'Session ended. Sign in as the next operator.' }, 200);
  ...
  try {
    ...
    upstream = await fetch(authApiUrl('/auth/switch-user'), {
      method: 'POST',
      headers,
      cache: 'no-store',
    });
  ...
  if (!upstream.ok) {
    ...
    const message = await boundedUpstreamMessage(upstream, 'Unable to switch users securely.');
