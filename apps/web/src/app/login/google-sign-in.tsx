...
import type { AuthenticatedSession, OrganizationChoice } from '@/lib/auth-contract';
...
export function GoogleSignIn({ onSelectionStateChange }: GoogleSignInProps) {
  ...
  function completeLogin(session: AuthenticatedSession): void {
    router.replace(
      session.context.organizationType === 'NONE' ? '/patient/dashboard' : '/dashboard',
    );
    router.refresh();
  }
  ...
  async function handleOrganizationSelection(membershipId: string): Promise<void> {
    ...
    setError('');
    try {
      const session = await selectGoogleOrganizationLogin({ idToken: credential, membershipId });
          ...
          {organizations.map((organization) => (
            <li key={organization.membershipId}>
              <button
                type="button"
                disabled={pending}
                onClick={() => void handleOrganizationSelection(organization.membershipId)}
                className="organization-theme-interactive w-full rounded-xl border border-[#10201c]/[.11] bg-[#fbfaf5] px-4 py-3.5 text-left text-sm text-[#10201c] disabled:cursor-wait disabled:opacity-60"
              >
