...
Proposed — Task 0010 implementation, pending CI and merge acceptance.
...
- Login first verifies the global identity and then derives active organization choices from memberships. Session issuance remains bound to one selected membership and tenant. Organization display metadata is returned without requiring a slug.
