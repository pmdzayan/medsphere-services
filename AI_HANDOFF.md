...
**Next feature work:** Review ADR-011 and the G3.10 sprint contract; do not implement
...
- G3.10 is proposed as a bounded physical batch-expiry reconciliation worker.
  It preserves on-hand quantity because expiry is not disposal, while expiring
  reservations that hold due batches. ADR-011 and the sprint contract are not
...
21. G3.10 physical batch expiry reconciliation — proposed; implementation
...
Review ADR-011 and
`docs/sprints/G3.10-physical-batch-expiry-reconciliation.md` from documentation
base `e1898cd`. Do not implement or advance progress until exact-commit contract
CI and CTO acceptance. Reservation creation, returns, quarantine/recall,
